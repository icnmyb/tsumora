...
- **Mリーグ成績の次の一手**: 麻雀ウォッチ等の記事ページは出場告知・速報文脈の確認に使い、成績値は M.LEAGUE公式Stats / M.LEAGUE応援まとめサイト / 公式発表系記事で二重確認する。次は Regular個人成績を `PlayerPhaseStats` に全40名分移し、`players/data.ts` の `annualPoints/currentSeason` と重複しない参照設計へ寄せる。
