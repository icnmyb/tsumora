...
- ✅ ホーム/Mリーグ以外のスマホUI再整備（指摘反映）
  ...
  - `/schedule` はスマホ見出しを「対局」に短縮し、週カレンダーの横切れを避けるためスマホでは本日の対局リストを主表示に変更
...
- **スマホ最終確認**: 2026-05-01の再指摘を受け、ホーム `/` と `/mleague` 以外は `/players`, `/schedule`, `/titles`, `/teams`, `/rules`, `/news` を中心に再調整済み。リリース前の実機確認では、ナビが全項目見えるか、スケジュールが右切れしないか、フィルタが押しやすいかを優先して見る。
...
- `app/schedule/page.tsx` / `app/globals.css`: スマホ見出しを「対局」に短縮し、スマホでは週カレンダーを隠して本日の対局リストを主役にする。対局行は時刻・状態・タイトルが読める2カラムカード寄りに調整。
