...
## 🟢 現在の状態（最終更新: 2026-09-16）

**この欄はセッション終わりに必ず更新する。次セッションで `/clear` しても引き継げるように。**

### 進行中タスク
- ✅ Mリーグ2026-27 レギュラーシーズン2日目結果反映
  - 2026-09-15の2試合を公式結果で更新。トップは尻無濱航 +64.9pt、茅森早香 +69.7pt
  - `app/mleague/season-data.ts` を全4試合終了時点へ更新し、セガサミーフェニックス +82.9ptの暫定首位、全10チーム順位、出場16選手の個人成績を反映
  - 9/15を次戦一覧から外し、次戦を9/17へ更新。`/mleague` のリード文も9/15終了時点へ更新
- ✅ `/schedule` Mリーグ2026-27 レギュラーシーズン全日程反映
  - M.LEAGUE公式発表・公式試合日程から、2026-09-14〜2027-03-02の全150卓（各卓2試合、全300試合）を `app/schedule/mleague-2026-27.ts` に追加
  - 2026-09-21〜2026-12-25の2卓同時開催日は、既存の「1開催日=1イベント」形式に合わせて1カードへ集約し、第1卓・第2卓の対戦チームを両方表示
  - 全10チーム各60卓（各120試合）、全94開催日、公式HTMLとの完全一致を検証
  - `tsc --noEmit` / `next build` 通過。`/schedule` の週送り、PC・390px幅、DOM、コンソールをブラウザ確認済み
- ✅ `/mleague` 順位表の品位改善
...
- ✅ ホームの対局セクション復活
  - 「日次の対局スケジュール・試合進行データは未整備」NOTEを削除
...
- ✅ タイトル戦一覧・ルール情報の再整備
  - `/titles` の掲載対象を各団体3件ずつ、計15タイトルへ拡張（JPMLは既存3件を維持）
  - 詳細ページの見出し「ルール / フォーマット」を「ルール」に変更
  - 各団体公式ページを確認し、JPML公式ルール、協会公式ルール、最高位戦/Classic、RMU A/B、μリーグ/μカップ系のルール差分を `app/titles/data.ts` に反映
  - ホームは日付依存表示のため `dynamic = "force-dynamic"` に変更
- ✅ `/schedule` カレンダー改善
  - 週表示は月曜始まりではなく、現在日の2日前から7日間を表示
  - 週カレンダー本体は横スクロール可能に変更
  - `?view=month` で月表示に切替可能。月表示も横スクロール可能
- ✅ `/schedule` NPM放送対局データ修正（2026-05-04）
  - 日本プロ麻雀協会公式スケジュール `https://npm2001.com/schedule/` から、8月までの「ABEMA」または「協会チャンネル」掲載対局を取り直し
  - 誤って 2026-05-04 に表示していた `雀王戦A1リーグ第2節A卓` を削除。公式上は 2026-04-18
  - NPM公式ページは詳細開始時刻が未掲載のため、既存方針どおり暫定 12:00–20:00 表示を維持
- ✅ `/schedule` 既存大会の放送予定を追加（2026-09-16）
  - 日本プロ麻雀協会公式スケジュールを2027年1月まで再確認し、既存掲載シリーズ（雀王戦・女流雀王戦）のうち2026-09-16以降で「ABEMA」または「協会チャンネル」と明記された21日程を追加
  - 最高位戦公式A1/A2卓組表の放送色を確認し、A1のABEMA放送10日程（プレーオフ2日を含む）とA2の最高位戦チャンネル放送8日程を追加。A2第11・12節は放送卓未定のため未掲載
  - オータムCS、雀竜位戦、チャンピオンロード、日本オープンなど、従来のスケジュールにない大会種別は追加していない
  - MリーグとJPMLの既存スケジュールデータは変更していない

### 直近の主要決定
- **UX判断軸**: とにかく細部までユーザーのことを考え抜く。目を引くことより、読んでいる人が迷わず、疲れず、気持ちよく理解できることを優先する。
- **計測方針**: Plausible で privacy-friendly analytics を使う。`app/layout.tsx` に Plausible の基本スクリプトは導入済み。`lib/analytics.ts` の `trackEvent` と `components/TrackedExternalLink.tsx` で手動イベントを送り、ナビクリック・外部リンク・動画再生・選手一覧のフィルタ/検索操作を計測する。検索語などの生データは送らず、長さ区分など最小限の props に留める。Plausible 側では custom event goals の作成が必要。
...
## 作業ログ: 2026-05-01 ホーム対局セクション復活

### 変更内容
- `app/page.tsx`: 古い未整備NOTEを削除し、`FINAL_2025_26.upcoming` から「本日の対局」「今すぐ視聴」を表示。
- `app/page.tsx`: 日付依存のため `dynamic = "force-dynamic"` を追加。
- `app/globals.css`: ホーム対局セクション用の2カラム/モバイル1カラム調整を追加。

### 検証
- `./node_modules/.bin/tsc --noEmit`: Codexランタイム Node で通過。
- `./node_modules/.bin/next build`: Codexランタイム Node で通過。
- dev server `http://localhost:3010/` のHTMLに「本日の対局」「今すぐ視聴」「Mリーグ ファイナル」「ABEMAで見る」が出て、古い未整備文言が消えていることを確認。
...
## 作業ログ: 2026-05-04 選手一覧の表示切り替え改善

### 変更内容
- `app/players/page.tsx`: `/players` の初期表示を「全選手」に変更し、「全選手 / Mリーガー」をタブで切り替える構成に整理。PC/スマホとも縦に2セクションを並べず、選んだ一覧だけ表示する。
- `app/globals.css`: 既存の紙面感、黒線、赤アクセントを保ったセグメント型タブに調整。スマホでは横幅いっぱい、PCでは本文幅に収まるサイズにした。

### 判断
ユーザーがまず探したい対象は全選手一覧なので、Mリーガーは専用タブで切り替える補助ビューにした。ページネーション中にMリーガータブへ切り替えた場合は `/players` に戻し、空表示にならないようにしている。

## 作業ログ: 2026-05-04 HOME広告バナー追加

### 変更内容
- `public/ads/sakurao-academy-desktop.png` / `public/ads/sakurao-academy-mobile.png`: 桜鳳アカデミー広告バナーをHOMEで使用。
- `app/page.tsx`: HOMEのトップ記事/Mリーグブロック直下に広告バナーを配置。スマホではmobile画像、PCではdesktop画像を出し分ける。
- `app/globals.css`: TSUMORAの黒線・影の質感に合わせた広告枠を追加。

### 判断
ファーストビューの編集導線を邪魔しない範囲で、スクロール開始直後に視認できる位置に置いた。リンク先URLは未指定のため、未確認リンクは付けていない。

## 作業ログ: 2026-05-04 MリーグFinal最終日開始時刻確認

### 変更内容
- `app/mleague/sf-data.ts`: `FINAL_2025_26.upcoming` の 2026-05-15 はすでに `17:00` で保持されていることを確認。
- `app/mleague/page.tsx` / `app/schedule/page.tsx`: 固定文言の「19:00〜」が最終日と矛盾しないよう、「通常19:00〜 / 最終日17:00〜」に変更。

### 運用
Codex automation `m-final-push` は 23:45 実行のまま維持する。

## 作業ログ: 2026-05-04 ニュース追加とHOME選択設計

### 変更内容
- `app/news/data.ts`: 「渡辺太、第34期麻雀マスターズ優勝」「Mリーグ2025-26ファイナル開幕」の記事を追加。
- `app/news/data.ts`: `HOME_NEWS_SELECTION` を追加し、HOMEの大きいニュース1件・小さいニュース2件をslugで手動選択できる設計に変更。
- `app/page.tsx`: HOMEのニュース表示を日付順自動選択ではなく `getHomeNewsSelection()` 参照へ変更。
- `app/news/page.tsx`: ニュース一覧は `publishedAt` 優先の新着順ヘルパー `getSortedNews()` を使うよう整理。

### 判断
HOMEの編集面は「最新順」よりも編集者が選ぶほうがTSUMORAらしいため、記事データと同じ場所で選択できる軽い手動運用にした。選択slugが消えても新着順で補完する。

### 追記
- `app/news/[slug]/page.tsx`: 記事詳細画面の「出典」欄は非表示に変更。`sources` は内部の確認履歴としてデータに残し、読者向けUIには関連リンクのみ表示する。
- `app/players/data.ts`: 渡辺太の獲得タイトルに `2026 第34期麻雀マスターズ` を追記。

## 作業ログ: 2026-09-16 JPML放送対局スケジュール更新

### 変更内容
- `app/schedule/page.tsx`: 日本プロ麻雀連盟の公式mellow-fanチャンネルに掲載された2026-09-16〜2027-01-09の放送予定から、既存大会に該当する35件を追加。Focus M season13、麻雀日本シリーズ、鳳凰戦B1、プロクイーン、王位戦、JPML WRC-Rなど、公式で配信枠が確認できる日のみ掲載した。九州プロリーグ、開幕式特別記念大会、学生麻雀王位戦、龍龍杯は対象外として除外した。
- `app/schedule/page.tsx`: 公式対戦表、公式YouTube、公式mellow-fanの配信枠を照合し、未掲載だった鳳凰戦A1リーグ第5節A卓〜最終節D卓の28件、A2リーグ第4節A卓〜最終節D卓の32件を追加した。
- 既存JPML枠のうち、公式配信ページで確定済みになった33件の出場者を実名へ更新。未確定の将来枠は疑問符ではなく「公式発表待ち」と明記した。
- 公式配信ページと差異があったWRC-R決勝の日付・開始時刻、小島武夫杯帝王戦とインターネット麻雀日本選手権の開始時刻、十段位決定戦初日・二日目の開始時刻、WRCベスト8の卓名を修正した。
- 公式APIは調査時の照合だけに使い、画面表示は従来どおりモジュールスコープの静的データから生成する。

### 検証
- 対象としたJPML公式配信予定35件のstream IDと、鳳凰戦A1・A2の追加60件がローカルデータに含まれることを確認。A1・A2はmellow-fan公式の過去配信27件・予約配信33件と日付・タイトルを全件突合し、追加漏れ・余計な追加・重複が0件、JPMLデータ内に `？？？？` が残っていないことを確認。
- `./node_modules/.bin/tsc --noEmit`: Codexランタイム Node で通過。
- `./node_modules/.bin/next build`: Codexランタイム Node で通過。
- アプリ内ブラウザで `/schedule` の当日Focus M season13、次週のFocus M、十段位決定戦二日目の確定選手名を確認。390px幅でも横方向のはみ出しなし、関連するコンソールエラーなし。
