...
## ホーム (`app/page.tsx`) の静的データ撲滅 — **新規12項目**

`/` トップページにはまだ捏造データやハードコード値が多数残っている。
故人 (前原雄大) の言及や事実誤認 (期数・移籍情報) を含むため、優先度高。

### ★★★ 即座に削除すべき (誤情報含む)

#### #H1 HEROトップストーリー削除
- "瀬戸熊、独走" + body + scoreboard で **前原雄大 (故人)** が登場
- 鳳凰位戦の偽スコア (瀬戸熊+124.5、前原+87.3 等)
- → 削除。代わりに「Featured Mリーガー紹介」「最新の動画ハイライト」など編集
  ヒーローに置き換え

#### #H2 EDITOR'S PICK 削除 or 動的化
- 「第42期鳳凰位戦、後半戦の焦点」偽記事
- 故人言及あり
- → 削除 or 編集記事を別途作る (CMS導入の必要性は低い、ハンドキュレでOK)

#### #H3 最新ニュース 6件の検証 / 削除
- 「高宮まり 協会→連盟」は誤 (既に連盟所属)
- 「渋川難波 協会→最高位戦」は2026時点で発生していない
- → 全削除 or 検証済み2-3件に絞る

### ★★ Group A: 既存データから動的計算可能

#### #H4 Mリーグ番付 sidebar の動的化
- 現状: 10チームのpts/順位ハードコード
- 改善: `/mleague` と同じ計算 (TEAMS + 各選手の annualPoints[2025-26] 合算)
- 共通ヘルパー `lib/mleague-standings.ts` に抽出する案

#### #H5 ランキングTable の動的化
- 現状: 滝沢18冠、金子15冠 等ハードコード (数値捏造)
- 改善: `/rankings` と同じ計算 (FEATURED_PLAYERS.titles[].length 降順 top 5)

#### #H6 タイトルカード 3枚の現実化
- 現状: 鳳凰位戦/雀王戦/Classic の暫定 standings ハードコード
- 改善: `app/titles/data.ts` の `pastChampions` から「**前期優勝者**」を表示
- 進行中の standings は諦める (データ無し)

### ★ Group C: Mリーグ DB (#G3) に依存

#### #H7 「本日の対局」5件の実データ化
- 試合スケジュールDB (`#G3`) と連動
- それまでは「**本日 試合データ準備中**」のhonest表示か、削除

#### #H8 7日カレンダーの実データ化
- 同上 (#G3 依存)
- 削除 or 「**スケジュール準備中**」表示

#### #H9 LIVE NOW sidebar の実データ化
- 同上
- 削除 or サイト全体の他のリッチコンテンツへの導線に置換

#### #H10 HEROスコアボードの実データ化
- 試合進行データが必要 (別途 ABEMA / 各団体データ取得)
- 短期的には #H1 と一緒に削除推奨

### 副次

#### #H11 故人 (前原雄大) の bio内mention の見直し
- player data.ts の bio文中に「前原雄大の影響で〜」等の言及残存
- 削除 or 「故 前原雄大」表記に変更検討
- 該当: 瀬戸熊の bio 内 1箇所

#### #H12 「金子正輝」の所属誤り
- ランキングテーブルで「金子正輝 (協会)」表記
- 金子正輝は 連盟所属 (KADOKAWAサクラナイツ)
- → #H5 動的化で自然に解消

### 実装順序 (推奨)
1. **#H1, #H2, #H3** (誤情報削除) — 1時間以内
2. **#H4, #H5, #H6** (既存data連動) — 半日
3. **#H7-#H10** (#G3 完了後)
4. **#H11, #H12** (上記の副次として自動解消)
