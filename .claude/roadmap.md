...
- ✅ ホームの古い未整備NOTEを削除し、`FINAL_2025_26.upcoming` から「本日の対局」「今すぐ視聴」セクションを復活
...
- ✅ 2026-05-04: NPM公式スケジュールを再確認し、8月までの `ABEMA` / `協会チャンネル` 掲載対局に差し替え。5/4に誤表示していた `雀王戦A1リーグ第2節A卓` は公式上 4/18 のため削除
- ✅ 2026-05-04: 連盟OPENREC `jpml0306` の配信予定を再確認し、OPENREC上で予定公開されている2026-05-05から2027-01-10までの対局・番組を `/schedule` に追加。終了時刻は未掲載のため、表示は開始時刻のみ、`endTime` はカレンダー表示用にFocus Mを3時間、それ以外を7時間で扱う
- ✅ 2026-05-04: MリーグFinal最終日 2026-05-15 の開始時刻は `FINAL_2025_26.upcoming` で `17:00` 保持済み。画面上の固定文言も「通常19:00〜 / 最終日17:00〜」へ修正
- ✅ 2026-05-04: `/titles` を各団体3件ずつ計15タイトルに拡張。タイトル詳細の「ルール / フォーマット」は「ルール」に変更し、JPML/NPM/最高位戦/RMU/μの公式ページ確認ベースでルール情報を更新
...
- 5/4リリースには必須ではない。現在の画面はチーム単位のRegular指標とSF個人成績があれば致命的な誤表示は避けられるため、リリース後に落ち着いて整備する

#### #M2 Final 開幕後の更新手順
- Final未開催のため、現在 `FINAL_2025_26` は持越ptのみ
- 2026-05-04 開幕後は `FINAL_2025_26.standings` と `PLAYER_PHASE_STATS_2025_26` の final レコードを、試合日ごとに確認済みソースから追加する
- 未確認の最高素点・率は入れず、確認できた項目だけ表示する
- 5/4当日の最低限は #R1 の更新導線確認。全試合分の履歴DB化・自動化はリリース後でよい
...
- 5/4リリースには必須ではない。まず内部データに出典を保持し、UI露出やGPT風回答への接続はリリース後に設計する

### ★★ 5/4後でよい — ニュース制作・編集ワークフロー本実装
...
- 5/4までに必要なのは #R3 の最小運用。本格的な編集ワークフロー、レビュー状態、テンプレート整備はリリース後でよい
...
#### #G10 試合結果を確認できる導線
- 試合終了後の結果を、ユーザーがどこかから一覧/履歴として確認できるようにする
- 候補: `/predict/results` の試合結果一覧、`/predict/me` の最近の結果、`/predict/match/[id]` から `/predict/result/[id]` への自動導線
- 表示内容: 試合日、節/試合番号、1着〜4着、素点、ユーザー自身の予想結果/獲得pt、関連する順位変動
- `#G4` の `match_results` / `prediction_scores` 登録後に、結果ページ単体ではなく「結果を探せる場所」まで含めて実装する
...
#### #G12 推し雀士ボーナス (案Aには現状なし)
- 案Aから外したが、追加するなら +30pt or X連携前 +50pt
- スコアロジック更新が必要

#### #G13 通知システム (推し選手出場・試合結果)
...
4. #G5 予想投稿
5. #G6 X連携トグル + #G7 ptルール表示 + #G10 試合結果導線
...
- HOMEの大きいニュース1件・小さいニュース2件は `app/news/data.ts` の `HOME_NEWS_SELECTION` でslug指定する。日付順の自動選択ではなく、編集者が選ぶ運用にする。
- ニュースの `sources` は内部確認用メタデータとして保持し、記事詳細UIには出典欄を出さない。
