...
async function main() {
  ...
  let withoutLeague = 0;
  for (const m of members) {
    if (EXCLUDED_PLAYER_NAMES.has(m.name)) {
      skipped++;
      continue;
    }
  ...
  console.error(`      → ${rosterEntries.length} roster entries (skipped ${skipped} featured/excluded, ${withLeague} with league, ${withoutLeague} without)`);
